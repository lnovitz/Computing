...
```{r, fig.cap="Figure 1: Variables With Visually High Correlation With Sale Price"}
...
First, I divided our dataset into a training and validation set. The code below details this transformation.
```{r}
set.seed(399)
...
head(House.subset)
```
Before building a model, I examined the linear relationship between each potential explanatory variable and the response variable (SalePrice) to make sure we could proceed with a linear model. 
...
$$
##The coefficients in our model relate to how much the response variable, SalesPrice, changes in regards to its corresponding variable, keeping all other variables constant. For example, for every point increase in overall quality a house is given, the SalesPrice is expected to increase by $17,000, keeping all other variables constant. In contrast, given that there's only 2 full bathroom in a house, the SalesPrice is expected to decrease by $20,000. So on and so forth. When there are 3 spaces for cars in a garage, the SalesPrice is expected to increase by $43,000, on average. 
...
```
Based on the lowest mean squared prediction error after performing 5-fold cross validation and forward selection, we choose the best model with 6 variables.
...
Obviously, we go with the former. 5-fold cross validation is always superior. 
...
```{r}
bestmods <- regsubsets(x = SalePrice ~ OverallQual + YearBuilt + YearRemodAdd + TotalBsmtSF + CentralAir + factor(Electrical) + FirstFlrSF + GrLivArea + factor(FullBath) + TotRmsAbvGrd + factor(GarageCars) + GarageArea,             
                   data = training %>% 
                     select(-grp),     
                   nvmax = 12,          
                   method = "forward") 
...
grps <- 5
varz <- 8
MSPE_cv <- matrix(nrow=6, ncol=8)
#ok
for (i in 1: grps){
  for (j in 1:varz){
    
test_cv <- house_data_cv %>% filter(grps == i)
validation_cv <- house_data_cv %>% filter(grps != i)

bestmods_cv <- regsubsets(x = SalePrice ~ OverallQual + YearBuilt + YearRemodAdd + TotalBsmtSF + CentralAir + Electrical + FirstFlrSF + GrLivArea + FullBath + TotRmsAbvGrd + GarageCars + GarageArea,
                   data = validation_cv %>% 
                     select(-grp),     
                   nvmax = 8,          
                   method = "forward") 
